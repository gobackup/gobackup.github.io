...
web:
  ...
  # tls:
    # certificate: /etc/ssl/gobackup.crt
    # private_key: /etc/ssl/gobackup.key
models:
...
| tls      | [TlsConfig](#tlsconfig) | Web server TLS Config           |             |

#### TlsConfig

> Since: 1.X.X 

WebUI can use HTTPS if you add a certificate and its private key in the configuration.

| Field       | Type   | Description                          | Default |
| ----------- | ------ | ------------------------------------ | ------- |
| certificate | string | Path to certificate file             |         |
| private_key | string | Path to certificate private key file |         |

You can obtain a self signed certificate with this exemple of OpenSSL command:
